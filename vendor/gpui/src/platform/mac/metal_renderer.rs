...
pub(crate) struct MetalRenderer {
    ...
    capture_texture: Option<metal::Texture>,
    capture_width: u32,
    capture_height: u32,
}
...
impl MetalRenderer {
    ...
    pub fn capture_frame(&self) -> Option<(u32, u32, Vec<u8>)> {
        let capture_tex = self.capture_texture.as_ref()?;
    ...
    pub fn draw(&mut self, scene: &Scene) {
        ...
        loop {
            ...
            match command_buffer {
                Ok(command_buffer) => {
                    // Blit drawable to capture texture for frame capture
                    let drawable_texture = drawable.texture();
                    let w = drawable_texture.width() as u32;
                    let h = drawable_texture.height() as u32;

                    // Recreate capture texture if size changed
                    if self.capture_width != w || self.capture_height != h || self.capture_texture.is_none() {
                        ...
                        self.capture_texture = Some(self.device.new_texture(&desc));
                        self.capture_width = w;
                        self.capture_height = h;
                    }

                    if let Some(ref capture_tex) = self.capture_texture {
                        ...
                        blit_encoder.synchronize_resource(capture_tex);
