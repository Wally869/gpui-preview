...
pub struct BladeRenderer {
    ...
    capture_buffer: Option<gpu::Buffer>,
    capture_width: u32,
    capture_height: u32,
}
...
impl BladeRenderer {
    ...
    pub fn draw(&mut self, scene: &Scene) {
        ...
        // Copy frame texture to capture buffer for readback
        let w = self.surface_config.size.width;
        let h = self.surface_config.size.height;
        let bytes_per_row = w * 4; // BGRA = 4 bytes per pixel
        let buffer_size = (bytes_per_row * h) as u64;

        if w > 0 && h > 0 {
            // Recreate capture buffer if size changed
            if self.capture_width != w || self.capture_height != h || self.capture_buffer.is_none() {
                if let Some(old_buf) = self.capture_buffer.take() {
                    self.gpu.destroy_buffer(old_buf);
                }
                self.capture_buffer = Some(self.gpu.create_buffer(gpu::BufferDesc {
                    name: "capture_readback",
                    size: buffer_size,
                    memory: gpu::Memory::Shared,
                }));
            ...
            if let Some(ref capture_buf) = self.capture_buffer {
                let mut transfer = self.command_encoder.transfer("capture_readback");
    ...
    pub fn capture_frame(&self) -> Option<(u32, u32, Vec<u8>)> {
        let capture_buf = self.capture_buffer.as_ref()?;
