...
impl Window {
    ...
    /// Capture the most recently rendered frame as BGRA8 pixel data.
    ///
    /// Returns `Some((width, height, pixels))` where `pixels` is a `Vec<u8>` of
    /// BGRA8 data (4 bytes per pixel, row-major), or `None` if capture is not
    /// supported on the current platform or no frame has been rendered yet.
    pub fn capture_frame(&self) -> Option<(u32, u32, Vec<u8>)> {
        self.platform_window.capture_frame()
    }
